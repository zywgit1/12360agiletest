				...
				submitHandler : function(form) {
					...
					var a = $('input[name="option"]:checked').val();
					 if (a === '票价优先') {
						...
						//var val = $("#select1").find("option:selected").text();
						var val = $("#leixing").find("option:selected").val();
						if (val === '高铁') {
							$.ajax({
								type: "post",
								url: base_url + '/highspeedrailwayPrior',
								data: JSON.stringify(json),
								contentType: 'application/json;charset=utf-8',
								dataType: 'json',
								success: function (data) {
						...
						else if(val === '其他车型')//除G D开头以外的
						{
							$.ajax({
								type: "post",
								url: base_url + '/otherPrior',
								data: JSON.stringify(json),
								contentType: 'application/json;charset=utf-8',
								dataType: 'json',
								success: function (data) {
	...
	$.ajax({
		...
		success : function(data) {
			if (data.stateCode == "200") {
				...
				$("#carNum").val(ticketItem[i].train_number);
				$("#orginLocation").val(ticketItem[i].start_station);
				$("#destinationLocation").val(ticketItem[i].reach_station);
				$("#startTime").val(ticketItem[i].start_time);
				$("#reachTime").val(ticketItem[i].reach_time);
				$("#ticketPrice").val(ticketItem[i].price);
	...
	if (data.stateCode === 200) {
		...
		for (var i = 0;i<data.data.length; i++) {
			...
			tr = '<td>'
				...
				+ '</td>'
				+ '<td>'
				+ data.data[i].train_number
				+ '</td>'
				+ '<td>'
				+ data.data[i].start_station
				+ '</td>'
				+ '<td>'
				+ data.data[i].reach_station
				+ '</td>'
				+ '<td>'
				+ data.data[i].start_time
				+ '</td>'
				+ '<td>'
				+ data.data[i].reach_time
				+ '</td>'
				+ '<td>'
				+ data.data[i].elapsed_time
				+ '</td>'
				+ '<td>'
				+ data.data[i].first_class
				+ '</td>'
				+ '<td>'
				+ data.data[i].second_class
				+ '</td>'
				+ '<td>'
				+ data.data[i].hard_seat
				+ '</td>'
				+ '<td>'
				+ data.data[i].none_seat
				+ '</td>'
				+ '<td>'
				+ data.data[i].price
				+ '</td>'
